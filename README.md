...
- 各プレイヤーは `sea` / `mountain` / `city` の 3 地形を、それぞれ 4 枚ずつ持って開始します。
...
4. 着地先に他プレイヤーのタイルがある場合、着地したタイルと同じ `地形` かつ同じ `オーナー` で連結しているタイル数ぶんのコインを支払います。
5. 支払い額が所持金を超える場合は、払えるだけ全額を支払います。
6. その後、同じ地形のタイル 2 枚を使って配置します。
...
- 支払い判定の連結は、「同じ地形」かつ「同じオーナー」でつながっている範囲のみを数えます。
